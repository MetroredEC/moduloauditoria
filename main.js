...
function persistUserDirectory() {
  localStorage.setItem(USER_STORAGE_KEY, JSON.stringify(userDirectory));
}
...
function loadUserDirectory() {
  try {
    ...
    userDirectory = sanitized.length ? sanitized : DEFAULT_USERS.map(sanitizeUserRecord).filter(Boolean);
...
function persistBrandingAssets() {
  localStorage.setItem(BRANDING_STORAGE_KEY, JSON.stringify(brandingAssets));
}
...
function updateBrandingUI() {
  const templateName = brandingAssets.reportTemplateName;
  setInlineStatus(
    'reportTemplateStatus',
    templateName ? `Plantilla cargada: ${templateName}` : 'Sin plantilla PDF cargada.',
    templateName ? 'success' : 'muted'
  );
...
function initBrandingConfig() {
  const templateInput = document.getElementById('uploadReportTemplate');
  if (templateInput) {
    templateInput.addEventListener('change', async (e) => {
      const file = e.target.files?.[0];
      if (!file) return;
      if (!/pdf$/i.test(file.name) && file.type !== 'application/pdf') {
        setInlineStatus('reportTemplateStatus', 'Formato inválido, cargue un PDF.', 'error');
        return;
      }
      const data = await readFileAsDataUrl(file);
      brandingAssets = { ...brandingAssets, reportTemplateName: file.name, reportTemplateData: data };
  ...
  const clearTemplateBtn = document.getElementById('clearReportTemplateBtn');
  if (clearTemplateBtn) {
    clearTemplateBtn.addEventListener('click', () => {
      brandingAssets = { ...brandingAssets, reportTemplateName: '', reportTemplateData: '' };
      persistBrandingAssets();
      updateBrandingUI();
    });
  }
...
function buildReportHeader(reportDate) {
  const logoBlock = brandingAssets.logoData
    ...
    : '';
  const templatePill = brandingAssets.reportTemplateName
    ? `<span class="report-template-pill">Plantilla: ${brandingAssets.reportTemplateName}</span>`
    : '';
  return `
