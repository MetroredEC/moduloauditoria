...
function initAuditOptions() {
  ...
  arcData.forEach((r, idx) => {
    ...
    opt.value = idx;
    opt.textContent = `${r.admision} · ${r.especialidad}`;
...
function renderAuditForm(record) {
  ...
  SECTION_CONFIG.forEach((section) => {
    ...
    section.criteriaIds.forEach((id) => {
      ...
      row.innerHTML = `<strong>${crit.name}</strong><p class="muted">${crit.description || ''}</p>`;
...
function renderSavedAuditsTable() {
  ...
  Object.entries(audits).forEach(([admision, data]) => {
    ...
    resumeBtn.onclick = () => {
      ...
      if (idx >= 0) {
        document.getElementById('selectRecord').value = idx;
