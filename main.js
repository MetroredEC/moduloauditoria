...
function renderAuditAssignmentTable() {
  const tbody = document.querySelector('#auditAssignmentTable tbody');
  if (!tbody) return;
  tbody.innerHTML = '';
  ...
  if (!filtered.length) {
    const tr = document.createElement('tr');
    tr.innerHTML = '<td colspan="9" class="muted">No hay historias asignadas con los filtros aplicados.</td>';
