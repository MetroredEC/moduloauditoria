...
const USER_DISPLAY = {
  gnavas: 'Argenis Navas',
  mvargas: 'Dra. Maite Vargas',
  nnavarrete: 'Nathalia Navarrete',
};
...
function renderAuditReport(record) {
  ...
  const areaAuditada = `Atención médica ambulatoria brindada al paciente en ${record.centro_medico || '_____'} en ${
    record.especialidad || '_____'
  }.`;
  const objetivo =
    ...
    <div class="report-page">
      ...
      <div class="report-section">
        ...
        <table class="report-signatures">
          ...
          <tr>
            <td><strong>${getDisplayName('gnavas')}</strong></td>
            <td><strong>${auditorName}</strong></td>
            <td><strong>${record.nombre_medico || 'Profesional auditado'}</strong></td>
          </tr>
        </table>
      </div>
    </div>
  ...
  container.innerHTML = `${pageOne}${REPORT_SECOND_PAGE}`;
}
...
function printReport() {
  ...
  win.document.write(`<!DOCTYPE html><html><head><title>Informe de auditoría</title><link rel="stylesheet" href="style.css" /></head><body>${content.innerHTML}</body></html>`);
  win.document.close();
  win.focus();
  win.print();
}
