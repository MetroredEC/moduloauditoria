...
function filterRecordsForUser(records) {
  if (!assignments.length) return records;
...
function renderRecordOptions() {
  ...
  const filtered = filterRecordsForUser(getRecordPool()).filter((r) => {
...
function renderAssignmentsTable() {
  ...
  if (!assignments.length) {
    const tr = document.createElement('tr');
    tr.innerHTML = '<td colspan="8" class="muted">Genere asignaciones desde la muestra estratificada.</td>';
    tbody.appendChild(tr);
    return;
  }

  assignments.forEach((rec, idx) => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
