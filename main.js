...
function showSection(id) {
  ...
  if (!allowed.includes(id) && window.location.hash !== `#${targetSection}`) {
  ...
  document
    ...
    .forEach((link) => link.classList.toggle('active', link.dataset.section === targetSection));
...
function updateNavigationByRole() {
  ...
  document.querySelectorAll('main .panel').forEach((panel) => {
    const allowed = ROLES[currentRole]?.includes(panel.id);
