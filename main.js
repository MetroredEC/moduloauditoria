...
function renderAuditAssignmentTable() {
  const pendingBody = document.querySelector('#pendingAuditsTable tbody');
  const completedBody = document.querySelector('#completedAuditsTable tbody');
  ...
  filtered.forEach((rec) => {
    const audit = audits[rec.admision];
    if (audit && audit.status === 'finalizada') completed.push(rec);
    else pending.push(rec);
  });

  updateAuditSummary(filtered.length, pending.length, completed.length);

  const buildRow = (rec, meta) => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
    ...
    actionBtn.onclick = () => openAuditFromTable(rec);
    tr.querySelector('.assign-cell').appendChild(actionBtn);
    return tr;
  };

  const renderTable = (rows, body, emptyText, metaResolver) => {
    if (!rows.length) {
      const tr = document.createElement('tr');
      tr.innerHTML = `<td colspan="9" class="muted">${emptyText}</td>`;
      body.appendChild(tr);
      return;
    }
    rows.forEach((rec) => {
      const meta = metaResolver(rec);
      body.appendChild(buildRow(rec, meta));
    });
  };

  renderTable(pending, pendingBody, 'No hay historias pendientes con los filtros aplicados.', (rec) => getAuditStatusMeta(rec.admision));
  renderTable(
    completed,
    completedBody,
    'Aún no tienes auditorías finalizadas con estos filtros.',
    () => ({ label: 'Finalizada', className: 'done', action: 'Ver auditoría' })
  );
}
...
function setAuditFormVisibility(show) {
  const card = document.getElementById('auditFormCard');
  if (!card) return;
