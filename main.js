...
const { safeStorage, loadUsers, saveUsers, sanitizeUserRecord, buildFullName, getCurrentUser, clearCurrentUser } =
  window.Auth || {};
...
function persistUserDirectory() {
  if (saveUsers) {
    userDirectory = saveUsers(userDirectory);
  }
}

function loadUserDirectory() {
  userDirectory = loadUsers ? loadUsers() : [];
  userRoleMap = {};
  userDisplayMap = {};
  userDirectory.forEach((u) => {
    userRoleMap[u.username] = u.role;
    userDisplayMap[u.username] = buildFullName ? buildFullName(u) : `${u.prefix} ${u.firstName} ${u.lastName}`.trim();
  });
}
...
function getDisplayName(username) {
  ...
  if (session && session.username === username && session.fullName) return session.fullName;
...
// Lee la sesión actual almacenada sin contraseña (compatibilidad con sesión legada).
function readSession() {
  const current = getCurrentUser ? getCurrentUser() : null;
  if (current) return current;
  try {
    const legacyRaw = safeStorage?.get('metroredSession');
    if (legacyRaw) {
      const parsed = JSON.parse(legacyRaw);
      if (parsed && parsed.username && parsed.role) {
        const fullName = userDisplayMap[parsed.username] || parsed.fullName || parsed.username;
        const session = { username: parsed.username, role: parsed.role, fullName };
        return session;
      }
    }
  } catch (e) {
...
// Limpia cualquier rastro de sesión al cerrar sesión.
function clearSession() {
  if (clearCurrentUser) clearCurrentUser();
  try {
    safeStorage?.remove('metroredSession');
  } catch (e) {
    // ignore
  }
}
...
function showSection(id) {
  ...
  const targetSection = allowed.includes(id) ? id : getDefaultSectionForRole(currentRole);
  if (window.location.hash !== `#${targetSection}`) {
    window.location.hash = `#${targetSection}`;
  }
  document.querySelectorAll('main .panel').forEach((sec) => {
    const canSee = allowed.includes(sec.id);
    sec.style.display = canSee ? '' : 'none';
    sec.classList.toggle('visible', sec.id === targetSection);
  });
  document.querySelectorAll('.main-nav a').forEach((link) => {
    const canSee = allowed.includes(link.dataset.section);
    link.style.display = canSee ? 'inline-flex' : 'none';
...
// Valida la sesión en index.html y aplica el rol activo en el header/nav.
function enforceSession() {
  ...
  currentRole = session.role || userRoleMap[session.username];
  const roleSelect = document.getElementById('roleSelect');
  const activeUserLabel = document.getElementById('activeUserLabel');
  const roleBadge = document.getElementById('roleBadge');
  const displayName = session.fullName || getDisplayName(session.username);
  if (roleSelect) {
...
function updateNavigationByRole() {
  ...
  document.querySelectorAll('main .panel').forEach((panel) => {
    const canSee = allowed.includes(panel.id);
    panel.style.display = canSee ? '' : 'none';
  });
  const requested = (window.location.hash || '').replace('#', '') || getDefaultSectionForRole(currentRole);
  const target = allowed.includes(requested) ? requested : getDefaultSectionForRole(currentRole);
  showSection(target);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('hashchange', () => {
    if (!isAuthenticated || !currentRole) return;
    const requested = (window.location.hash || '').replace('#', '') || getDefaultSectionForRole(currentRole);
    const target = ROLES[currentRole]?.includes(requested) ? requested : getDefaultSectionForRole(currentRole);
    showSection(target);
  });
  initAdminTabs();
  initBrandingConfig();
