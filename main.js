...
function renderAuditForm(record) {
  ...
  const nonConformities = document.getElementById('auditNonconformities');
  comments.value = stored.comentarios;
  recommendations.value = stored.recomendaciones;
  nonConformities.value = stored.noConformidades;
  comments.oninput = () => {
  ...
  nonConformities.oninput = () => {
    const existing = getOrInitAudit(record.admision);
    audits[record.admision] = { ...existing, noConformidades: nonConformities.value, fecha: new Date().toISOString() };
    persistAudits();
  };
...
function autoFillNonConformity(record, critId, value) {
  const area = document.getElementById('auditNonconformities');
  ...
  if (value === 0) {
    ...
    lines = lines.filter((l) => l !== phrase);
  }
  area.value = lines.join('\n');
  const existing = getOrInitAudit(record.admision);
  audits[record.admision] = { ...existing, noConformidades: area.value, fecha: new Date().toISOString() };
  persistAudits();
}
...
function saveAudit(record, status = 'borrador') {
  ...
  const noConformidades = document.getElementById('auditNonconformities').value.trim();
...
function restoreAudit(record) {
  ...
  document.getElementById('auditNonconformities').value = data?.noConformidades || '';
