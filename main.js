...
function renderAuditAssignmentTable() {
  ...
  rows
    .sort((a, b) => (statusOrder[a.meta.className] || 0) - (statusOrder[b.meta.className] || 0))
    .forEach(({ rec, meta }) => {
      const tr = document.createElement('tr');
      tr.innerHTML = `
...
function showAuditStartModal(record, meta = getAuditStatusMeta(record?.admision)) {
  ...
  const actionLabel = meta?.action?.toLowerCase() || 'gestionar';
  if (title) title.textContent = meta?.className === 'done' ? 'Ver auditoría' : 'Confirmar inicio';
  if (body) {
    body.innerHTML = `
      <p>Confirma para ${actionLabel} la historia seleccionada.</p>
...
function confirmAuditStart() {
  if (!modalRecord) return;
  openAuditFromTable(modalRecord);
...
function initAuditModal() {
  ...
  if (confirmBtn) confirmBtn.addEventListener('click', confirmAuditStart);
}
