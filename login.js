...
// Fallback store for contexts donde localStorage no está disponible (modo incógnito, políticas estrictas).
const memoryStore = {};
const safeStorage = {
  get: (key) => {
    try {
...
function loadUsers() {
  try {
    ...
    if (version !== CURRENT_USER_VERSION) {
      safeStorage.set(USER_STORAGE_KEY, JSON.stringify(finalUsers));
    ...
    const defaults = DEFAULT_USERS.map(sanitizeUser).filter(Boolean);
    safeStorage.set(USER_STORAGE_KEY, JSON.stringify(defaults));
    safeStorage.set(USER_VERSION_KEY, CURRENT_USER_VERSION);
    return defaults;
  }
}
...
function saveSession(username, role) {
  ...
  safeStorage.set('metroredSession', JSON.stringify(session));
}
...
(function bootstrapLogin() {
  loadUsers();
  const existing = safeStorage.get('metroredSession');
